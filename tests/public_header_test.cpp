...
#undef ROLLTUI_API
    ...
    const int kPublic = 318, kInternal_ = 575, kDelete = 0;
...
#undef ROLLTUI_API
    ...
    check(rt["VOCAB"] == 33 && rt["HOST_LOAD"] == 28 && rt["HOST_SETTINGS"] == 55 &&
              rt["HOST_BIND"] == 87 && rt["HOST_RUN"] == 81 && rt["HOST_RELEASE"] == 7 &&
              rt["TOOL_INTEROP"] == 0 && rt["WIDGET"] == 27,
          "the roles are the recorded shape — vocab 33, host load 28 / settings 55 / bind 87 / run 81 / "
