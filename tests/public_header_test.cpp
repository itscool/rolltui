...
#undef ROLLTUI_API
    ...
    const int kPublic = 309, kInternal_ = 571, kDelete = 0;
...
#undef ROLLTUI_API
    ...
    check(rt["VOCAB"] == 33 && rt["HOST_LOAD"] == 28 && rt["HOST_SETTINGS"] == 48 &&
              rt["HOST_BIND"] == 86 && rt["HOST_RUN"] == 81 && rt["HOST_RELEASE"] == 7 &&
              rt["TOOL_INTEROP"] == 0 && rt["WIDGET"] == 26,
          "the roles are the recorded shape — vocab 33, host load 28 / settings 48 / bind 86 / run 80 / "
