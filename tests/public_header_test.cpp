...
int main() {
  ...
  const std::size_t kInternalHeaders = 34; /* +rolltui_theme_editor.h; +rolltui_context.h; -rolltui_app_profile.h */
...
#undef ROLLTUI_API
    ...
    const int kPublic = 301, kInternal_ = 553, kDelete = 0;
...
#undef ROLLTUI_API
    ...
    check(rt["VOCAB"] == 33 && rt["HOST_LOAD"] == 28 && rt["HOST_SETTINGS"] == 46 &&
              rt["HOST_BIND"] == 86 && rt["HOST_RUN"] == 76 && rt["HOST_RELEASE"] == 6 &&
              rt["TOOL_INTEROP"] == 0 && rt["WIDGET"] == 26,
          "the roles are the recorded shape — vocab 33, host load 28 / settings 46 / bind 86 / run 76 / "
