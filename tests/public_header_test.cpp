...
#undef ROLLTUI_API
    ...
    const int kPublic = 322, kInternal_ = 579, kDelete = 0;
...
#undef ROLLTUI_API
    ...
    check(rt["VOCAB"] == 33 && rt["HOST_LOAD"] == 30 && rt["HOST_SETTINGS"] == 55 &&
              rt["HOST_BIND"] == 89 && rt["HOST_RUN"] == 81 && rt["HOST_RELEASE"] == 7 &&
              rt["TOOL_INTEROP"] == 0 && rt["WIDGET"] == 27,
          "the roles are the recorded shape — vocab 33, host load 30 / settings 55 / bind 89 / run 81 / "
