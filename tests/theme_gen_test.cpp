...
// declaration order, since a theme's style table is indexed by this ordinal. Only
// `role_name()` is needed here (generate()'s own "broken" list prints role names), so the
// reverse lookup `role_from_name` is not reproduced. ----
