...
int main() {
  ...
  for (const UcdCase& c : cases) {
    std::vector<bool> got = word_boundaries(c.cps);
  ...
  // ---- word_range: the double-click ----
  {
    ...
    auto word = [&](std::size_t off) { ByteRange r = word_range(s, off); return s.substr(r.begin, r.end - r.begin); };
    ...
    check(word_range(s, s.size()).begin == s.size(), "an offset at the end selects nothing");
    check(word_range("", 0).begin == 0 && word_range("", 0).end == 0, "empty input");
    ByteRange u = word_range("a_b c", 1);
    check(u.begin == 0 && u.end == 3, "underscore is ExtendNumLet: a_b is one word (WB13a/b)");
  }

  // ---- strip_escape_sequences ----
  {
    check(strip_escape_sequences("plain \xC3\xA9 text\n") == "plain \xC3\xA9 text\n", "text without escapes passes byte for byte");
    check(strip_escape_sequences("a\x1b[31mred\x1b[0mb") == "aredb", "CSI colour sequences are removed whole (not just the ESC)");
    check(strip_escape_sequences("x\x1b]0;title\x07y") == "xy", "OSC terminated by BEL is removed with its payload");
    check(strip_escape_sequences("x\x1b]8;;http://e\x1b\\link\x1b]8;;\x1b\\y") == "xlinky", "OSC 8 hyperlinks: the wrapping sequences go, the text stays");
    check(strip_escape_sequences("x\x1bPq#0;2;0;0;0\x1b\\y") == "xy", "a DCS string is removed to its ST");
    check(strip_escape_sequences("x\x1b(By") == "xy", "ESC + intermediate + final (charset designation) is removed");
    check(strip_escape_sequences("x\x1b" "7y\x1b" "8z") == "xyz", "two-byte ESC sequences (save/restore cursor) are removed");
    check(strip_escape_sequences("x\x1b") == "x", "a trailing lone ESC is dropped");
    check(strip_escape_sequences("x\x1b\ny") == "x\ny", "ESC before a control: only the ESC goes");
    check(strip_escape_sequences("x\xC2\x9B" "2Jy") == "xy", "the 8-bit CSI (U+009B) form is removed too");
    check(strip_escape_sequences("x\x1b]unterminated") == "x", "an unterminated string sequence swallows to the end (never renders as text)");
    check(strip_escape_sequences("\xFF\xFE ok") == "\xFF\xFE ok", "invalid UTF-8 passes through untouched (the decoder shows U+FFFD later)");
  }
