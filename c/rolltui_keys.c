...
#define ROLLTUI_PROTOCOL_NAME_(lower, UPPER, Camel) lower,
    ROLLTUI_PROTOCOL_LIST(ROLLTUI_PROTOCOL_NAME_)
#undef ROLLTUI_PROTOCOL_NAME_
};
 ...
 * the chord, so a person is told what to turn on rather than that something is impossible —
 * `Keys.cpp`'s own comment, moved with the words it described. */
static const char* const kUndeliverableText[] = {
    "",
    "it is not a key",
...
const char* rolltui_key_undeliverable_text(int code, size_t* len) {
  const size_t n = sizeof kUndeliverableText / sizeof kUndeliverableText[0];
  const char* s = (code >= 0 && (size_t)code < n) ? kUndeliverableText[code] : "";
  if (len) *len = strlen(s);
  return s;
}
...
typedef struct { const char* display; const char* file; } KeyNamePair;
static const KeyNamePair kKeyNamePairs[] = {
#define ROLLTUI_KEY_NAMES_(UPPER, Title, file) {Title, file},
    ROLLTUI_KEY_LIST(ROLLTUI_KEY_NAMES_)
#undef ROLLTUI_KEY_NAMES_
};
...
int rolltui_key_from_display_name(const char* name, size_t len) {
  size_t i, j;
  if (!name) return -1;
  for (i = 0; i < ROLLTUI_KEY_COUNT; ++i) {
    const char* lit = kKeyNamePairs[i].display;
    for (j = 0; j < len; ++j) {
